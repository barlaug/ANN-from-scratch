######################################################################################################################################
## This file consists of utility functions for training the neural net, as well as som global variables decribing the ann-structure ##
######################################################################################################################################
import random
import math 
import utilities.helpers_matrix as hm
from read_data import train_x
...
def init_weights_and_bias(layer1, layer2):
    ...
    if len(w) == 1: # True if weight is for a single output node
        return w[0], b[0]
    else: 
        return w, b
...
def feed_forward(x, weights, biases):
    ...
    """
    # Since our neural net is quite basic, we can do this without almost any loops and logic; improves readability.
...
def backpropagate(x, y, weights, biases, a):
    ...
    # Now comes a fairly long expression, we therefore divide it into two subcalcluations, with corresponding subscripts in the variable name
...
def train(x_train, y_train, weights, biases, a, num_epochs=100, tol_err=0.05):
    ...
    returns:
        ...
        biases: tuned biases
    """
    error_trajectory = [] # To keep track of the loss at output
    i = 0
    while (i < num_epochs):
        errors_i = []
        for k, xk in enumerate(x_train):
            yk = y_train[k]
            # Step 1: Feed through and evaluate:
            err = 0.5*((feed_forward(xk, weights, biases) - yk)**2)
            errors_i.append(err)
            # Step 2: Backpropagate and update weights/biases:
        ...
        if mean_error_i < tol_err:
            print(f"Finished. Error below predefined tolerance ({tol_err}).")
            break
